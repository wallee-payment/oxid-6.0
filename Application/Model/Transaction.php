...
class Transaction extends \OxidEsales\Eshop\Core\Model\BaseModel
{
    ...
    protected $_aSkipSaveFields = ['oxtimestamp', 'WLEVERSION', 'WLEUPDATED'];
    ...
    private function getTransactionLabels()
    {
    	...
    	if ($this->getSdkTransaction()->getPaymentConnectorConfiguration()) {
    		if ($this->getSdkTransaction()->getPaymentConnectorConfiguration()->getPaymentMethodConfiguration()) {
    			$paymentDescription = WalleeModule::instance()->WalleeTranslate($this->getSdkTransaction()->getPaymentConnectorConfiguration()->getPaymentMethodConfiguration()->getResolvedDescription());
    			$paymentMethod = WalleeModule::instance()->WalleeTranslate($this->getSdkTransaction()->getPaymentConnectorConfiguration()->getPaymentMethodConfiguration()->getResolvedTitle());
    		}
    		else {
        ...
        $labels = array(
            ...
            'labelGroup' => array(
                array(
                    'title' => WalleeModule::instance()->translate("Transaction #!id", true, array('!id'=>$this->getTransactionId())),
                    'labels' => array(
                        array(
                        ...
                        array(
                            ...
                            'description' => WalleeModule::instance()->translate('The amount which was authorized with the wallee transaction.'),
        ...
        foreach ($jobs as $job) {
            $jobLabels = array(
                ...
                array(
                    'title' => WalleeModule::instance()->translate('wallee Link'),
                    'description' => WalleeModule::instance()->translate('Open in your wallee backend.'),
                    'value' =>  $this->getWalleeLink($this->getJobLinkType($job), $job->getLinkedSpaceId(), $job->getId(),  WalleeModule::instance()->translate('Open'))
                )
            );
            ...
            $labelGroup[$job->getId()] = array(
                'title' =>WalleeModule::instance()->translate($message, true, array('!id' => $job->getId())),
                'labels' => $jobLabels
            );
        }
        return $labelGroup;
